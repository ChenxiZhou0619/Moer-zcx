#include "WeightedDelta.h"
#include <FunctionLayer/Material/Material.h>

WeightedDeltaTracking::WeightedDeltaTracking(const Json &json)
    : VolumetricPathTracer(json) {
  maxDepth = fetchOptional(json, "maxPathLength", 5);
}

Spectrum WeightedDeltaTracking::li(const Ray &_ray, const Scene &scene,
                                   std::shared_ptr<Sampler> sampler) const {
  auto tr_tracker_regular = [](const Medium *medium, Ray ray, float tmax) {
    return medium->Transmittance_ResidualRatioTracking(ray, tmax);
  };
...
REGISTER_CLASS(WeightedDeltaTracking, "weightedDeltaTracking")